...
def _extract_next_cursor(payload: Any) -> str | None:
    """QueryResponse carries `next_cursor` via Pydantic extra=allow. Pull it
    off model_extra rather than relying on a typed attribute so this keeps
    working if the SDK promotes the field to a real attribute later."""
